...
def define_UNet(modelpath,img_ch):
    model = Optim_U_Net(img_ch=img_ch,output_ch=2)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        for i in range(n_blocks):       # add ResNet blocks
            resnet9 += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]

        upsampling1 = []
        for i in range(n_downsampling):  # add upsampling layers (for translated domain images)
            mult = 2 ** (n_downsampling - i)
            upsampling1 += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=use_bias),
                      norm_layer(int(ngf * mult / 2)),
                      nn.ReLU(True)]
        upsampling1 += [nn.ReflectionPad2d(3)]
        upsampling1 += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        upsampling1 += [nn.Tanh()]
        
        upsampling2 = []
        for i in range(n_downsampling):  # add upsampling layers (for translated domain labels)
            mult = 2 ** (n_downsampling - i)
            upsampling2 += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=use_bias),
                      norm_layer(int(ngf * mult / 2)),
                      nn.ReLU(True)]
        upsampling2 += [nn.ReflectionPad2d(3)]
        upsampling2 += [nn.Conv2d(ngf, 1, kernel_size=7, padding=0)]
        upsampling2 += [nn.Tanh()]
        
        self.downsampling = nn.Sequential(*downsampling)
        self.resnet9 = nn.Sequential(*resnet9)
        self.upsampling1 = nn.Sequential(*upsampling1)
        self.upsampling2 = nn.Sequential(*upsampling2)
        
    def forward(self, input):
        """Standard forward"""
        output = self.downsampling(input)
        output = self.resnet9(output)
        output1 = self.upsampling1(output)
        output2 = self.upsampling2(output)
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        unet_block2 = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block2 = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block2, norm_layer=norm_layer)
        unet_block2 = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block2, norm_layer=norm_layer)
        self.model2 = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block2, outermost=True, norm_layer=norm_layer)  # add the outermost layer
    def forward(self, input):
        """Standard forward"""
...
class Optim_U_Net(nn.Module):
    def __init__(self,img_ch=1,output_ch=2,filter_size = 32):
        ...
        self.Conv_1x1 = nn.Conv2d(filter_size,2,kernel_size=1,stride=1,padding=0)
        
        self.Conv_2x2 = nn.Conv2d(filter_size*2,2,kernel_size=1,stride=1,padding=0)
        
        self.UpS1 = up_conv2(ch_in=filter_size*16,ch_out=filter_size*4,factor=4)
        self.UpS2 = up_conv2(ch_in=filter_size*4,ch_out=filter_size,factor=4)
        
        self.Conv_AxA = nn.Conv2d(filter_size,3,kernel_size=1,stride=1,padding=0)
